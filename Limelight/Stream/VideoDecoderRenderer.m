    ...
    while (LiPollNextVideoFrame(&handle, &du)) {
        ...
        if (ret == DR_OK) {
            ...
            if (MLScrollTraceIsEnabled() && MLScrollTraceIsAwaitingRender()) {
                MLScrollTraceSnapshot trace = MLScrollTraceCompleteRender(renderSampleNowMs);
                if (trace.traceId != 0 && trace.lastDispatchAmount != 0 && trace.lastDispatchMs != 0) {
                    unsigned long long startAgeMs = trace.startedMs != 0 && renderSampleNowMs >= trace.startedMs ? renderSampleNowMs - trace.startedMs : 0;
                    unsigned long long localAgeMs = renderSampleNowMs >= trace.lastDispatchMs ? renderSampleNowMs - trace.lastDispatchMs : 0;
                    Log(LOG_D, @"[inputdiag] scroll-trace render trace=%llu axis=%@ amount=%d highRes=%d frame=%d startAge=%llums localAge=%llums pending=%d",
                        (unsigned long long)trace.traceId,
                        trace.lastDispatchHorizontal ? @"H" : @"V",
                        (int)trace.lastDispatchAmount,
                        trace.lastDispatchHighRes ? 1 : 0,
                        du->frameNumber,
                        startAgeMs,
                        localAgeMs,
                        LiGetPendingVideoFrames());
